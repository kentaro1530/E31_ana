...
const TString outrootfile="~/spectra_fit2.root";
const Kamano kamanoSpectra;
TGraphErrors* g_fit[3];

void fcn_I1_A(Int_t &npar, Double_t *gin, Double_t &chi2, Double_t *par, Int_t iflag);
void fcn_I1_B(Int_t &npar, Double_t *gin, Double_t &chi2, Double_t *par, Int_t iflag);
void get_chi2_ndf_I1_A(Double_t &chi2, Int_t &ndf, Double_t *par);
void get_chi2_ndf_I1_B(Double_t &chi2, Int_t &ndf, Double_t *par);
bool fit_I1(TMinuit *minuit, double &par, double &err);
TMinuit* makeMinuit_I1();
...
int main(){
  ...
  {
    TDirectory *dir=(TDirectory*)of-> mkdir("fit_model_B_2");
    dir-> cd();
    TMinuit* minuit_I1=makeMinuit_I1();
    minuit_I1-> SetFCN(fcn_I1_B);
    double par0, err0;
    fit_I1(minuit_I1, par0, err0);
    double par_I1[1]={ par0 };
    double chi2; int ndf;
    get_chi2_ndf_I1_B(chi2, ndf, par_I1);

    std::cout<<"I1 = "<<par_I1[0]<<" +- "<<err0<<std::endl;
    std::cout<<"> Chi2 = "<<chi2<<"   NDF = "<<ndf<<std::endl;

    TMinuit* minuit=makeMinuit();
    int iflag=0;
    minuit-> mnparm(1, "I_1", par0, 0.001, 0., 2., iflag);
...
void fcn_I1_A(Int_t &npar, Double_t *gin, Double_t &chi2, Double_t *par, Int_t iflag){
  int npoint=0;
  get_chi2_ndf_I1_A(chi2, npoint, par);
}

void fcn_I1_B(Int_t &npar, Double_t *gin, Double_t &chi2, Double_t *par, Int_t iflag){
  int npoint=0;
  get_chi2_ndf_I1_B(chi2, npoint, par);
}
...
void get_chi2_ndf_I1_A(Double_t &chi2, Int_t &ndf, Double_t *par){
  ...
  std::vector<double> pimS0_y;
  for( size_t i=0; i<I1_xy[0].size(); i++ ){
    pimS0_y.push_back(par[0]*I1_xy[1][i]);
  }
  ...
  for( int i=0; i<g_fit[2]->GetN(); i++ ){  
    ...
    double calc_y=reso::conv_kp(x, I1_xy[0], pimS0_y);
...
std::vector<TGraph*> make_spectra_A(double par[3]){
  ...
  for( size_t i=0; i<I0_xy[0].size(); i++ ){
    ...
    pimS0_y.push_back(par[1]*I1_xy[1][i]);
    interfer_y.push_back(0.5*par[2]*par[1]*par[0]*interfer_xy[1][i]);
  }  
...
TMinuit* makeMinuit_I1(){
  ...
  minuit-> SetPrintLevel(-1);
  TString par_name[1]={ "Scale I=1" };
  double step[1]={ 0.01 };
  double init_par[1]={1.0 };
  double min[1]={ 0 };
  double max[1]={ 2.0 };
  int flag=0;
  for( int i=0; i<1; i++ ) minuit-> mnparm(i, par_name[i], init_par[i], step[i], min[i], max[i], flag);
  return minuit;
}
...
bool fit_I1(TMinuit *minuit, double &par, double &err){
  ...
  for( int i=0; i<minuit->GetNumPars(); i++ ){
    TString name; double bnd1, bnd2; int flag;
    minuit-> mnpout(i, name, par, err, bnd1, bnd2, flag);
  }
  return true;
}
