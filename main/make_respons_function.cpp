...
const TString outputname="respons_function.root";
...
int main(){
  ...
  for(double W2=START_MASS; W2<END_MASS; W2+=DELTA_MASS ){ // W2=pi Sigma mass
    //  for(double W2=piMass+sigmaMass; W2<1.6; W2+=0.01 ){
	...
	TLorentzVector inflight_lmom=beam_lmom+target_lmom-n_lmom-offshell_lmom;
	TVector3 boost=-(onshell_lmom+beam_lmom).BoostVector();
	...
	const double G0=1.0/(inflight_lmom_at_KN.Vect().Mag2()-pow(inflight_lmom_at_KN.M(), 2));
