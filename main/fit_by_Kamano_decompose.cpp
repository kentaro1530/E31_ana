...
const TString outrootfile="~/spectra_fit_decompose.root";
...
int main(){
  ...
  g_fit[0]=(TGraphErrors*)f->Get("I0_CS");
  g_fit[1]=(TGraphErrors*)f->Get("sub_CS0");
  g_fit[2]=(TGraphErrors*)f->Get("pimS0_CS0");

  TFile *of=new TFile(outrootfile, "recreate");
  of-> cd();
  g_fit[0]-> Write("gra_I0_CS");
  g_fit[1]-> Write("gra_sub_CS");
  g_fit[2]-> Write("gra_pimS0_CS");

  ((TGraphErrors*)f->Get("pimSp_CS0"))-> Write("gra_pimSp_CS");
  ((TGraphErrors*)f->Get("pipSm_CS0"))-> Write("gra_pipSm_CS");
...
void get_chi2_ndf_A(Double_t &chi2, Int_t &ndf, Double_t *par){
  ...
  for( int i=0; i<g_fit[0]->GetN(); i++ ){  
    ...
    double calc_y=par[0]*reso::conv_kn(x, I0_xy[0], I0_xy[1]);
  ...
  for( int i=0; i<g_fit[1]->GetN(); i++ ){  
    ...
    double calc_y=par[2]*reso::conv_kn(x, I0_xy[0], interfer_xy[1]);
    //    std::cout<<i<<"  "<<y<<"  "<<calc_y<<"  "<<err<<std::endl;
    if( err<1.0e-8 ) continue;
  ...
  for( int i=0; i<g_fit[2]->GetN(); i++ ){  
    ...
    double calc_y=par[1]*reso::conv_kp(x, I0_xy[0], I1_xy[1]);
    if( err<1.0e-8 ) continue;
