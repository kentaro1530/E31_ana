...
const TString infileroot="~/spectra_fit2.root";
const TString outfile="~/fit_model_B.root";

void draw_fit_B_fix_phase(){
  init();
  TFile *f=new TFile(infileroot);
  TFile *of=new TFile(outfile, "update");
  draw(f, "fit_model_B_2");
  printParam(f, "fit_model_B_2");
}
...
void draw(TFile *f, TString dirname){
  ...
  gra_pimS0-> SetLineColor(3);
  gra_I1-> SetLineColor(3);
