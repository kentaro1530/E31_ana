...
void draw_decomposed_DCC(){
  ...
  leg-> SetBorderSize(0);
  leg-> SetFillStyle(0);
  leg-> AddEntry(gra_I0, "Data", "lep");
  leg-> AddEntry(gra_I0_calc_A, "Model.A", "lp");
  leg-> AddEntry(gra_I0_calc_B, "Model.B", "lp");
  set(gra_sub);
  set(gra_I0);

  set(gra_I0_calc_A, kRed);
  set(gra_I1_calc_A, kRed);
  set(gra_interfer_calc_A, kRed);

  set(gra_I0_calc_B, kBlue);
  set(gra_I1_calc_B, kBlue);
  set(gra_interfer_calc_B, kBlue);
  ...
  gra_I0_calc_A-> Draw("PL");
  leg-> Draw();
  c1-> Print("~/ag_Run78/pic/Dron/comp_kamano_I0.eps");
  return; 

  gra_sub-> Draw("AP");
  gra_interfer_calc_B-> Draw("PL");
  gra_interfer_calc_A-> Draw("PL");
  c1-> Print("~/ag_Run78/pic/Dron/comp_kamano_interfer.eps");

  gra_pimS0-> GetYaxis()-> SetRangeUser(0.0, 8.0);
  gra_pimS0-> Draw("AP");
  gra_I1_calc_B-> Draw("PL");
  gra_I1_calc_A-> Draw("PL");
  c1-> Print("~/ag_Run78/pic/Dron/comp_kamano_I1.eps");
  return;
  ...
  c1-> Print("~/ag_Run78/pic/JPS_2022/interfer_fit.eps");

  gra_pimS0-> Draw("AP");
  gra_I1_calc-> Draw("PL");
  line.DrawLine(kpThre, 0.0, kpThre, 30);
  c1-> Print("~/ag_Run78/pic/JPS_2022/I1_fit.eps");

  gra_I0-> Draw("AP");
  gra_I0_calc-> Draw("PL");
  line.DrawLine(kpThre, 0.0, kpThre, 30);
  c1-> Print("~/ag_Run78/pic/JPS_2022/I0_fit.eps");

  gra_pimSp-> Draw("AP");
  gra_pimSp_calc-> Draw("PL");
  line.DrawLine(kpThre, 0.0, kpThre, 30);
  c1-> Print("~/ag_Run78/pic/JPS_2022/DCC_pimSp_B_fit.eps");

  gra_pipSm-> Draw("AP");
  gra_pipSm_calc-> Draw("PL");
  line.DrawLine(kpThre, 0.0, kpThre, 30);
  c1-> Print("~/ag_Run78/pic/JPS_2022/DCC_pipSm_B_fit.eps");
}
