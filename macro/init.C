...
TGraphErrors* GevToMev(TGraphErrors* gra, int colorID=kBlack){
  ...
  TGraphErrors* gra2= new TGraphErrors(x.size(), &x[0], &y[0], &xerr[0], &yerr[0]);

  gra2-> SetLineColor(colorID);
  gra2-> SetMarkerColor(colorID);
  gra2-> SetMarkerStyle(8);
  gra2-> SetLineWidth(3);
  gra2-> SetMarkerSize(1.2);
  gra2-> GetXaxis()-> SetRangeUser(1350, 1500);
  gra2-> GetYaxis()-> SetRangeUser(0, 30);
  gra2-> SetTitle();
  gra2->GetXaxis()->SetLabelSize(0.05);
  gra2->GetXaxis()->SetTitleSize(0.06);
  gra2->GetXaxis()->CenterTitle();

  gra2->GetYaxis()->SetLabelSize(0.05);
  gra2->GetYaxis()->SetTitleSize(0.06);
  gra2->GetYaxis()->CenterTitle();
  gra2->SetTitle("");
  return gra2;
}
